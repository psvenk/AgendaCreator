...
function drawTable(daysInWeek, table, inputOrOutput, classes, numClasses, dayDescs) {
    ...
    (function () {
        if (inputOrOutput == InputOrOutput.output) {
            var date = parseDate(document.getElementById("datePicker").value);
            if (typeof date != "undefined") {
                ...
                if (arrayEquals(daysInWeek, DaysInWeek["5"]) ||
                    ...
                    var dateDay = DaysInWeek["7-js-order"][date.getDay()];
                    /* String representation of the day of the week selected
                     * in the date picker
                     */
                    var dayIndex = DaysInWeek["7"].indexOf(dateDay);
                    if (dayIndex > 0) {
                ...
                for (var i = 0; i < daysInWeek.length; i++) {
                    headerRow2.appendChild(document.createElement("th")).
                        textContent += date.addDays(i).toLocaleDateString("en-US", {
                        "month": "short",
                        "day": "numeric"
                    });
                }
 ...
 * @license MPL-2.0+
 */
document.getElementById("enableDates").addEventListener("click", function () {
...
function serialize() {
    var obj = {};
    obj.version = "0.3.1";
    // The Node.js build script will change "0.3.1" to the current version
